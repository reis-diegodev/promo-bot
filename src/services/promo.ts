...
import { generateMeliAffiliateLink } from '../modules/affiliate/affiliate.service';
...
async function buildAffiliateUrl(
  ...
  if (storeName === 'Mercado Livre') {
    return await generateMeliAffiliateLink(url);
  }
...
export async function processAndSendPromos(
  ...
  for (const promo of promos) {
    try {
      ...
      console.log(
        `✅ Adicionado à fila (${sentCount}) | Próximo em ${delay / 60000}min`,
      );
